package com.tfjybj.framework.auth.util.validate;
...
public abstract class RegexNewUtils {
    //用于匹配任意字符串的非贪婪表达式
    ...
    public static Map<String, Object> simpleMatchToRegexpMap(Map<String, Object> map) {
        for (Map.Entry<String, Object> entry : map.entrySet()) {
            Object value = entry.getValue();
            if (value instanceof String) {
                String escapeValue = simpleMatchToRegexp((String) value);
    ...
    public static Pattern simplePattern(String key, boolean IgnoreCase, boolean matchStart, boolean matchEnd, boolean wildcardSpace) {
        ...
        regStr = ((matchStart) ? "" : RegexNewUtils.MATCH_ANY_STRING) + ((wildcardSpace) ? key.replace(" ", "\\s+") : regStr) + ((matchEnd) ? "" : RegexNewUtils.MATCH_ANY_STRING);
